...
## Create dataframe for state-level iv regressions 
...
make_ivregdata <- function(outlier,startdate,enddate,incstart,incend){
  ...
  # Add controls - county pop 06
  
  countypop06 <- read.csv("data/countypop06.csv") %>% 
    dplyr::select(STATE,COUNTY,POPESTIMATE2006) %>% 
    filter(COUNTY!=0) %>% 
    mutate(county=sprintf("%03d",COUNTY)) %>% 
    mutate(FIPS=as.numeric(paste0(STATE,county,sep=""))) %>% 
    dplyr::select(FIPS,POPESTIMATE2006)
  
  totals06countfin <- left_join(totals06countfin,countypop06)
  
  totals06count3 <- totals06countfin %>% 
    group_by(State) %>% 
    mutate(totalinvsum06= sum(totalval_sum06),
           statepop06 = sum(POPESTIMATE2006),
           totalinvsum06_lag=sum(total06val_sum_lag)) %>%
    group_by(State,newdummn) %>% 
    mutate(groupsum06= sum(totalval_sum06),
           grouppop06 = sum(POPESTIMATE2006),
...
# Add state unemployment 2006 
stateunemp06 <- read.csv("data/stateunemp2006.csv",stringsAsFactors = FALSE,skip=7)

stateunemp062 <- stateunemp06 %>% 
  filter(X.3=="Total") %>% 
  dplyr::select(X.2,X.10) %>% 
  rename(state=X.2,
         unemprate06=X.10) %>%
  mutate(unemprate06=as.numeric(unemprate06 ))


fmapjun11wo <- left_join(fmapjun11wo,stateunemp062)

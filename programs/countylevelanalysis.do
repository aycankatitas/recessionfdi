...
use "analysis_data/countylevel.dta",clear
...
** TABLE A12 - ECONOMIC DRIVERS OF COUNTY STATUS 
...
esttab sr1 sr2 sr3 using output/countylevel/mnlstimincen.tex,  se(2) b(2) label unstack noomitted noconstant nobaselevels replace  parentheses stats(N state, labels("Observations" "State-Fixed Effects")) ///
  ...
  addnotes("Multinomial logit coefficients estimated via maximum likelihood. The outcome is a categorical variable that indicates  never, old and new county status. The baseline category is never counties. Model 1 includes all real-time incentives, Model 2 includes grants and training reimbursements, and Model 3 includes enterprise zones and infrastructure assistance. All models include state-fixed effects. Robust standard errors are in parentheses and clustered by state. * p$<$0.10, ** p$<$0.05, *** p$<$0.010") 
    
eststo clear 

** TABLE A13 - PLACEBO TEST TAX INCENTIVES 
capture estimates drop *
qui {
...
** TABLE A14 - POLITICAL DRIVERS OF COUNTY STATUS - competitive X layoff 
capture estimates drop *
qui {
...
** TABLE A15 - POLITICAL DRIVERS OF COUNTY STATUS - competitive x eligibility 
...
** TABLE A16 - COUNTY IV FIRST STAGE RESULTS 
...
qui{
	...
	est sto ivf, title("Education Stimulus (Scaled)")
	estadd local state "yes", replace
}
...
** TABLE A17 - County IV Second-stage results 
capture estimate drop *
qui{
...
* TABLE A18 - COUNTY COMMUNITY COLLEGE 
...
* TABLE A19 - DIFFERENT TYPES OF STIMULUS
...
esttab a1 a2 a3 a4 using output/countylevel/mnldifferentstim.tex,  se(2) b(2) label unstack noomitted noconstant replace  parentheses stats(N state, labels("Observations" "State-Fixed Effects")) ///
  ...
  substitute([htbp] [!htbp] {l} {p{\linewidth}}) ///
  addnotes("Multinomial logit coefficients estimated via maximum likelihood. The dependent variables are categorical variables that indicate never, old and new county status. The baseline category is never counties. An election is competitive if the difference in two-party vote share is less than 10 percentage-points. All models include state-fixed effects. Robust standard errors are in parentheses and clustered by state. * p$<$0.10, ** p$<$0.05, *** p$<$0.010") 
  
*** TABLE A23 - COUNTY MARKET SIZE
...
esttab ms1 using output/countylevel/mnlmarketsize.tex,  se(2) b(2) label unstack noomitted noconstant replace  parentheses stats(N state, labels("Observations" "State-Fixed Effects")) ///
  ...
  starlevels( * 0.10 ** 0.05 *** 0.010) ///
  substitute([htbp] [!htbp] {l} {p{\linewidth}}) ///
  addnotes("Multinomial logit coefficients estimated via maximum likelihood. The dependent variable is a categorical variable that indicates a never, old and new county status. The baseline category is never counties. An election is competitive if the difference in two-party vote share is less than 10 percentage-points. Model 1 and 2 include state-fixed effects. GDP per capita measure is available only for metropolitan statistical areas. Robust standard errors are in parentheses and clustered by state. * p$<$0.10, ** p$<$0.05, *** p$<$0.010") 
 
  
** TABLE A20 - EXCLUDING SOLAR ENERGY COMPANIES
...
label variable logtotaldom "Domestic Greenfield Investment (Logged) 2008"
label variable substimeduc_log "Education Stimulus (Logged)"
label var real_incentives_log "Real-Time Incentives (Logged)"
label var comp "Competitive County"
label var comp2 "Competitiveness"
label var governor "Republican Governor"
...
esttab a5 using output/countylevel/mnlsolar.tex,  se(2) b(2) label unstack noomitted noconstant replace  parentheses stats(N state, labels("Observations" "State-Fixed Effects")) ///
  ...
  starlevels( * 0.10 ** 0.05 *** 0.010) ///
  substitute([htbp] [!htbp] {l} {p{\linewidth}}) ///
  addnotes("Multinomial logit coefficients estimated via maximum likelihood. The dependent variable is a categorical variable that indicates a never, old and new county status. The baseline category is never counties. An election is competitive if the difference in two-party vote share is less than 10 percentage-points. Model includes state-fixed effects. The sample excludes investment in renewable energy industry. Robust standard errors are in parentheses and clustered by state. * p$<$0.10, ** p$<$0.05, *** p$<$0.010") 
  

** TABLE A22 - EXCLUDING TAX HAVENS
...
lab var unemptot "Unemployment Rate 2008"
